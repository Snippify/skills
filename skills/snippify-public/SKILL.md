...
4. Call `get_artifact` with a selected public Artifact ID or an ID from `my` to retrieve its approved current version text and file metadata.
...
- `artifacts` contains active public Artifacts; `my` contains only the authenticated user's active private Artifacts. Both require an approved current version. Team Artifacts and drafts are not returned.
