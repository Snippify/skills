...
description: Maintain a tiny project-local journal for Snippify Artifact retrievals and file suggestions. Use with Snippify read or contribution actions to reuse prior results and avoid repeated MCP calls and context.
---

# Snippify Journal

Keep `.snippify/journal.tsv` in the current project as a compact cache. Create it automatically after a successful `get_artifact` or file-based `suggest_artifact_version`; do not ask merely to maintain this journal.
...
- After search/list reveals a candidate version, run `check-get --artifact ID --version VERSION`. Exit code 0 means the same candidate was already materialized at an existing project path, so reuse that file unless the user requests a refresh. Exit code 1 means call `get_artifact`.
...
- Search with two to five discriminating words. Use list metadata to choose one Artifact before fetching its full candidate.
...
```text
python3 <skill-dir>/scripts/journal.py record-get --project <root> --artifact <artifact-id> --version <candidate-version-id> --path <project-path-or-dash>
